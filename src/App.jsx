...
export default function App() {
  ...
  // Estados para la sesión y autenticación
  ...
  // Cargar los pacientes del usuario activo
  useEffect(() => {
    if (session) {
      fetchPacientes();
    } else {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (isEditing) {
      // Excluir campos autogenerados para evitar conflictos en la actualización
      const { id, created_at, user_id, ...datosParaActualizar } = formData;
      const { error } = await supabase.from('pacientes').update(datosParaActualizar).eq('id', editId);
      ...
      // Asociar explícitamente el ID de usuario autenticado
    ...
    return (
      <div className="min-h-screen bg-slate-200 flex items-center justify-center font-sans">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-slate-600 font-bold text-lg">Cargando...</p>
        </div>
      </div>
    );
  }
  ...
  if (!session) {
    return (
      <div className="min-h-screen bg-slate-200 flex items-center justify-center p-4 font-sans">
        <div className="w-full max-w-md bg-white p-8 md:p-10 rounded-3xl shadow-xl shadow-slate-400/20 border border-slate-300">
          <div className="flex flex-col items-center mb-8">
            <div className="p-4 bg-blue-600 rounded-2xl shadow-lg shadow-blue-500/30 mb-4 text-white">
              <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
              </svg>
            </div>
            <h2 className="text-3xl font-black text-slate-800 tracking-tight">Fisiologica</h2>
            <p className="text-slate-500 font-semibold mt-2 text-center text-sm">
              {isSignUp ? 'Registra tu cuenta de terapeuta' : 'Inicia sesión para acceder a tus pacientes'}
            </p>
          </div>
          ...
          <form onSubmit={handleAuth} className="space-y-5">
            {authError && (
              <div className="bg-red-50 border border-red-200 text-red-700 p-3.5 rounded-xl text-sm font-medium">
                {authError === 'Invalid login credentials' ? 'Credenciales de inicio de sesión no válidas' : authError}
              </div>
            )}
            <div>
            ...
            <div>
              <label className={labelEstilo}>Contraseña</label>
              <input 
                type="password" 
                required 
                value={password} 
            ...
            <button 
              type="submit" 
              className="w-full bg-blue-600 hover:bg-blue-700 text-white font-extrabold py-4 rounded-xl shadow-lg shadow-blue-600/30 border border-blue-800 transform hover:-translate-y-0.5 transition-all duration-200 text-lg uppercase tracking-wide cursor-pointer"
            >
  ...
  return (
    <div className="min-h-screen bg-slate-200 p-4 md:p-8 flex flex-col gap-6 font-sans">
      
      {/* HEADER DE LA APLICACIÓN */}
      <header className="bg-white p-5 md:p-6 rounded-3xl shadow-xl shadow-slate-400/20 border border-slate-300 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-800 tracking-tight leading-none">Fisiologica</h1>
            <p className="text-xs text-slate-500 font-semibold mt-1">Gestión de Historias Clínicas</p>
          </div>
        </div>
        
        <div className="flex flex-col sm:flex-row items-center gap-4">
          <div className="text-center sm:text-right">
            <p className="text-[10px] text-slate-400 font-extrabold uppercase tracking-widest">Terapeuta Activo</p>
            <p className="text-sm font-bold text-slate-700">{session.user.email}</p>
          </div>
          <button 
            onClick={() => supabase.auth.signOut()} 
            className="bg-red-50 hover:bg-red-100 text-red-600 hover:text-red-700 px-4 py-2.5 rounded-xl text-xs font-bold transition border border-red-200 shadow-sm cursor-pointer"
          >
            Cerrar Sesión
          </button>
        </div>
      ...
      {/* SECCIÓN PRINCIPAL: FORMULARIO Y DIRECTORIO */}
      <div className="flex flex-col xl:flex-row gap-8">
        ...
        <div className="xl:w-2/3 bg-white p-6 md:p-10 rounded-3xl shadow-xl shadow-slate-400/20 border border-slate-300">
          <div className="flex items-center justify-between mb-8 border-b border-slate-200 pb-6">
            ...
            {isEditing && (
              <button onClick={cancelarEdicion} type="button" className="text-sm bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-2 px-4 rounded-lg transition cursor-pointer">
                Cancelar Edición
              </button>
            )}
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <button type="submit" className={`w-full text-white font-extrabold py-4 rounded-xl shadow-lg transform hover:-translate-y-0.5 transition-all duration-200 text-lg uppercase tracking-wide border cursor-pointer ${isEditing ? 'bg-amber-500 hover:bg-amber-600 shadow-amber-500/30 border-amber-600' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-600/30 border-blue-800'}`}>
                ...
                return (
                  <div key={paciente.id} className={`p-5 rounded-2xl border transition-all duration-200 border-l-4 group ${incompleto ? 'bg-amber-50 border-amber-200 hover:border-amber-400 border-l-amber-500' : 'bg-slate-50 border-slate-200 hover:border-blue-300 border-l-blue-600'}`}>
                    ...
                    <div className="flex justify-end gap-2 mt-4">
                      <button onClick={() => cargarParaEditar(paciente)} className="bg-white border border-slate-300 text-slate-600 hover:text-blue-600 hover:border-blue-300 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors shadow-sm cursor-pointer">
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                        Editar
                      </button>
                      <button onClick={() => eliminarPaciente(paciente.id)} className="bg-white border border-slate-300 text-slate-600 hover:text-red-600 hover:border-red-300 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors shadow-sm cursor-pointer">
